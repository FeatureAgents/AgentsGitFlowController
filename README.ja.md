...
| 適用範囲 | リポジトリの全ユーザー（人間を含む） | フックまたはプラグインを設定した AI エージェント（人間の直接操作は制限されない） |
| 強制タイミング | サーバー側、プッシュ / マージ時 | ローカル側、コマンド実行前 |
| プラットフォーム依存 | 特定のホスティングサービスに依存 | 純粋なローカル Git、プラットフォーム非依存（`gh` / `glab` は任意） |
| バイパス可能性 | 管理者権限を持つユーザー | ガードに配線されていないエージェント、または高度に悪意あるエージェント |
...
本ガードは各プラットフォームのツール実行前イベント — DSH の `tools/pre-execute`、Pi の `tool_call`、CLI クライアントの `PreToolUse` フック —（コマンドがディスパッチされる直前の決定ポイント）にフックします。ここで `deny` されたコマンドは **一切実行されません**。エージェントは拒否結果のみを受け取ります。事後検出（ログスキャン）は強制手段として機能しません — 被害はすでに発生してしまっているからです。
...
```
`gitflow-guard wire --client opencode` がこのファイルをパッケージから書き込みます。手書きは推奨されません。
...
他の 3 つの CLI クライアントも同じフック形式を使用します — `wire` がそれぞれのファイルを書き込みます：

- **CodeBuddy** — `.codebuddy/settings.json`
- **ZCode** — `.zcode/config.json`（`hooks.enabled: true` も設定）
...
- **PATH とバイナリの解決**: グローバルインストールにより `gitflow-guard` バイナリが提供されます。`wire` はすべてのフックを、インストール済みパッケージ自身の runner の絶対パスにアンカーするため、エージェントの実行環境が対話型の `PATH` を継承していない場合でもフックは動作し続けます。
...
現在チェックアウトされているブランチ（`git branch --show-current`）、および `pr merge` / `mr merge` 実行時の PR/MR ターゲット（`gh pr view` / `glab mr view` 経由）のみです。任意の `worktree` ガードを有効にした場合は、`git status --porcelain`（ダーティ / 未追跡の状態）も読み取り、`requireUpstreamSynced` が設定されている場合は `git rev-list --left-right --count HEAD...@{upstream}` も読み取ります。モデルは順序駆動ではなく **役割駆動**（ターゲットがどの役割のブランチか）であるため、コミットの祖先関係を解析する必要はありません。
...
- **新しいエージェントプラットフォームの統合**: Windsurf、次世代 CLI エージェントなど、新興ツールのフック/拡張機構の調査と適合。
