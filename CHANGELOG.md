...
- feat(platform): add Cursor AI coding agent integration — add `cursor` platform support to `HookPlatform` and `ClientId`; implement payload extraction (`beforeShellExecution` / `preToolUse` with `command`, `cwd`, `cursor_version`, and `workspace_roots`), UTF-8 BOM stripping, `encodeDeny` (`exit 0` with stdout JSON `{"permission":"deny","user_message":...,"agent_message":...}`), and `gitflow-guard wire --client cursor` with idempotent `.cursor/hooks.json` configuration handling; update verification matrix with section `[J]` (77 tests passing); synchronize references, E2E test cases, design docs, dogfood configurations, and all 11 language READMEs —— 新增 Cursor AI 编程智能体接入支持: `HookPlatform` 与 `ClientId` 扩展支持 `cursor`；实现 payload 提取（`beforeShellExecution` / `preToolUse` 命令、`cwd`、`cursor_version` 与 `workspace_roots`）、UTF-8 BOM 头安全剥离、`encodeDeny`（exit 0 + stdout JSON `{"permission":"deny","user_message":...,"agent_message":...}`）以及 `gitflow-guard wire --client cursor` 自动接线（支持幂等写入 `.cursor/hooks.json` `version: 1` 与 `hooks.beforeShellExecution` 结构）；连续复测矩阵新增 `[J]` 测试套件（77 项全绿）；同步更新参考文档、E2E 测试用例、设计文档、dogfood 配置及全部 11 语 README。

## 0.0.40

- feat(platform): add CodeBuddy and ZCode AI coding agent integrations — add `codebuddy` and `zcode` platform support to `HookPlatform` and `ClientId`; implement payload extraction (`tool_input.command` + `cwd`), `encodeDeny` (exit 2 + stderr), and `gitflow-guard wire --client <codebuddy|zcode>` with idempotent `.codebuddy/settings.json` and `.zcode/config.json` configuration handling (including ZCode's mandatory `hooks.enabled: true` and `hooks.events.PreToolUse` nesting); update verification matrix with sections `[H]` and `[I]` (71 tests passing); synchronize references, E2E test cases, design docs, dogfood configurations, and all 11 language READMEs —— 新增 CodeBuddy 与 ZCode AI 编程智能体接入支持: `HookPlatform` 与 `ClientId` 扩展支持 `codebuddy` 和 `zcode`；实现 payload 提取（`tool_input.command` + `cwd`）、`encodeDeny`（exit 2 + stderr）以及 `gitflow-guard wire --client <codebuddy|zcode>` 自动接线（支持幂等写入 `.codebuddy/settings.json` 与 `.zcode/config.json`，并严格适配 ZCode 必需的 `hooks.enabled: true` 与 `hooks.events.PreToolUse` 嵌套结构）；连续复测矩阵新增 `[H]` 与 `[I]` 测试套件（71 项全绿）；同步更新参考文档、E2E 测试用例、设计文档、dogfood 配置及全部 11 语 README。

## 0.0.39

- fix(classify): cap nested command expansion depth to prevent stack-overflow crash — `classify()` recursed without bound into every `$()` / backtick layer extracted by `extractNested`, so a pathological payload (`$($(...))` thousands deep) blew the call stack and killed the hook process with an uncaught `RangeError`; added `MAX_NESTED_DEPTH = 10` so expansion degrades to "stop unwrapping" past the cap while the outer command is still classified (aligned with the project's default fail-open policy on internal errors). The public `classify(command, ctx)` signature is unchanged — recursion moved into an internal `classifyDepth(command, ctx, depth)`. Also corrected the stale 🟠-3/🔵-12 status in `docs/issues.md` (both workflows already ran `check:readmes` and the contract test covers them) and documented the new cap in `docs/design.md` §6; the 10-level ceiling is also recorded as a known local limitation in the "locally non-defensible" paragraph of all 11 language READMEs —— 命令嵌套展开深度上限: `classify()` 此前对 `extractNested` 提取的每一层 `$()` / 反引号无限递归，病态载荷（数千层 `$($(...))`）会耗尽调用栈并以未捕获 `RangeError` 崩溃 hook 进程；新增 `MAX_NESTED_DEPTH = 10`，超限后降级为「不再展开内层」而外层命令照常分类（与项目内部异常默认 fail-open 的策略一致）。公开签名 `classify(command, ctx)` 保持不变，递归下沉到内部 `classifyDepth(command, ctx, depth)`。同时订正 `docs/issues.md` 中 🟠-3/🔵-12 的滞后状态（两个工作流均已执行 `check:readmes` 且契约测试已覆盖），并在 `docs/design.md` §6 补记该上限；该 10 层上限同时作为已知本地局限写入全部 11 语 README 的「本地不可防」说明段。
