...
**第 1 步——安装**。九个客户端全部使用同一个 npm 包 `agents-gitflow-guard`，按你的 agent 类型选择对应方式：

```bash
# 模式 A: CLI Hook 客户端 (Claude Code · Codex · OpenCode · Antigravity · CodeBuddy · ZCode · Cursor)
...
```bash
# Codex / OpenCode / Antigravity / CodeBuddy / ZCode / Cursor(各写各的配置文件; --yes 跳过 y/N 确认)
...
`wire` 对已有配置**非破坏性合并**(已存在的 hook 不动; 重跑 wire 会把旧版 gitflow-guard 条目原位迁移为当前自锚定形态),默认只写**当前工程目录**;`--global`(本机所有仓库)写入前必先确认或需 `--yes`。各客户端的文件与格式见[安装详解](#安装详解)。
...
- 每条分支条目是精确名或正则(自动识别)。**正则安全**:分支正则由项目作者提供并按原样编译,`featurePattern` 按整条分支名匹配(编译为 `^(?:<pattern>)$` 锚定),请写覆盖整串的表达式——`feature/`、裸 `fix` 这类子串式写法不会命中,分支会落到 `other`。`featurePattern` 与分支条目仍须避免灾难性回溯写法(如 `(\w+)+` 这类嵌套量词)。
...
|---|---|---|
| Claude Code · Codex · OpenCode · Antigravity · CodeBuddy · ZCode · Cursor | `npm i -g agents-gitflow-guard` | `gitflow-guard wire --client <名> --project --yes` |
...
### 1. CLI Hook 客户端 (Claude Code · Codex · OpenCode · Antigravity · CodeBuddy · ZCode · Cursor)
...
```bash
# A. CLI Hook 客户端 (Claude Code · Codex · OpenCode · Antigravity · CodeBuddy · ZCode · Cursor)
...
- **平台协议规范**: Hook 从 stdin 读取 payload 并按各平台规范响应：
  - **Claude Code / OpenCode / CodeBuddy / ZCode**：`exit 2`（stderr 输出原因与指引）。
  ...
  - **Cursor**：stdout 输出 JSON `{"permission":"deny","user_message":...,"agent_message":...}` 且 `exit 0`。
...
在其支持的命令形态内,角色边界在本地强制生效:合入受保护角色分支(integration / preview / production / archive)必须走配置好的路径(PR/MR,或生产/归档的人工合并)。常见混淆包装已纳入分类与拦截——shell 包装(`sh -c` / `bash -lc`)、子 shell 与反引号/`$()` 内嵌、`env`/`command`/`nohup`/`xargs`/`sudo` 前缀与 `VAR=x` 赋值、绝对路径、管道与 `||` 后半段、git 全局选项(`-C .`、`--git-dir=…`)、内联定义的 git 别名(`-c alias.<名>=push …`、`git config alias.<名> …`,含加引号、大小写变体与链式形态)、通配 refspec(`refs/heads/*:refs/heads/*`)、当 fetch+merge 用的 `git pull`,以及 `send-pack`/`update-ref`/`symbolic-ref` 等 plumbing;强制重建受保护分支(`checkout -B`/`switch -C`)与受保护分支上的 cherry-pick/revert 由 ref-update / ref-move 门禁拦截。可执行对抗语料见 `tests/accuracy-audit.spec.ts`。

已知**本地不可防**的通道:直连 forge API(`gh api repos/…/pulls/N/merge`、`curl`)、解释器子进程内嵌(`node -e "child_process.exec('git push …')"`),以及文本解析看不见的别名——已存在于 `.git/config` 的、经环境变量通道注入的、用 shell 变量或命令替换拼出的、写成 ANSI-C/反斜杠转义的,以及在一条命令段中定义、在后续段中调用的。任意深度的引号/编码变换天然只能尽力而为;`$()` 或反引号嵌套超过 10 层后不再展开(解析器停止展开, 而不是在病态载荷上崩溃)。真正不可绕过的边界在你托管服务的分支保护设置。两边都用——把本守卫当作即时反馈与审计留痕,而不是安全边界。
...
npm run test:git-matrix # 169 项 git 决策矩阵(对真实仓库执行)
