...
> 0.0.12 已由用户推 tag 发布(P1-3 原始挂起解除); 整改内容经版本提交落在本分支为 **0.0.13**, 待 PR 合并后推 tag v0.0.13 发布。
> 流程勘误留档: 整改中途曾基于过期快照误判「tag 未推送」而重指本地 v0.0.12——已回滚还原对齐远端, 改走 0.0.13 独立发版。
...
- **状态**: ✅ 已解决(2026-08-22)——用户已推 tag v0.0.12, CI 发布完成(npm dist-tags.latest=0.0.12 实测确认)。第三轮整改内容另起 **0.0.13**: CHANGELOG 独立条目、package.json bump、双语 README 锁版本示例同步 @0.0.13、版本提交与 annotated tag v0.0.13 落在本分支(§2 流程), 待 PR 合并后由用户推 tag 发布。勘误: 整改中途曾误重指本地 v0.0.12(基于审查时的过期 registry 快照), 已删除并 fetch 还原对齐远端——已发布的 tag 不可移动。
