...
> 测试脚本已固化于 **gfguard-e2e**(`scripts/gfguard-matrix.sh` / `gfguard-realflow.sh` / `gfguard-pi-cases.sh`),本文件用例与之一一对应。

## 前置条件

- 本机 `pi` CLI 可用且已登录(`~/.pi/agent/auth.json`;版本记录于 TestResult)。
- 受控测试仓库 = **gfguard-e2e 仓库本身**(master=integration / beta=preview / (fix|task)/*=feature;本地裸远端 `/tmp/gfguard-e2e-origin.git`)。
- 守卫已按真实用户安装路径装入:gfguard-e2e `package.json` devDependency + `.pi/settings.json` + `.pi/extensions/gitflow-guard.ts`。
- `GITFLOW_GUARD_BIN` 指向被测守卫(默认 `gfguard-e2e/node_modules/.bin/gitflow-guard`,可覆盖为本仓库 `bin/gitflow-guard.mjs`)。
...
```bash
# 决策矩阵(135 用例,CLI 级;需 GUARD_BIN 指向被测守卫)
GUARD_BIN=/path/to/AgentsGitFlowController/bin/gitflow-guard.mjs bash scripts/gfguard-matrix.sh
# 真实拦截(Pi 扩展通道,用例 A-D)
bash scripts/gfguard-pi-cases.sh
# 真实放行流(feature 全生命周期)
bash scripts/gfguard-realflow.sh
```
